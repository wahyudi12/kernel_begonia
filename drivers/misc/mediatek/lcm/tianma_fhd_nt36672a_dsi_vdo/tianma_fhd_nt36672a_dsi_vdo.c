...
static void lcm_set_disp_param(unsigned int param)
{
	...
	ret = panel_disp_param_send_lock(PANEL_TIANMA_NT36672A, param, push_table);
	return;
}
...
static int lcm_led_i2c_reg_op(char *buffer, int op, int count)
{
	...
	LCM_LOGI("%s,reg_val is %s reg_addr = %d\n", __func__, buffer, reg_addr);
	pr_info("pr_info%s,reg_val is %s reg_addr = %d\n", __func__, buffer, reg_addr);
...
struct LCM_DRIVER tianma_fhd_nt36672a_dsi_vdo_lcm_drv = {
	.name = "tianma_fhd_nt36672a_dsi_vdo_lcm_drv",
